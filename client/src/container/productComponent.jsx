...
import {
  ...
  fetchComments,
  deleteComment,
} from "../Redux/Review/Actions/reviewAction";
...
function ProductComponent(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (comentario.title.length > 30) {
      ...
      setError([
        ...error,
        "El titulo del comentario no puede tener más de 30 caracteres",
      ]);
      return;
    }
          ...
          <form onSubmit={handleSubmit}>
            <div className={cComponent.formContent}>
              <div className={cComponent.person}>
                ...
                <input
                  value={comentario.person}
                  onChange={handleInputChange}
                  placeholder="(Opcional)"
                  name="person"
                  className={cComponent.personInput}
                  type="text"
                />
              </div>
              <div className={cComponent.comment}>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  name="title"
                  className={cComponent.titleInput}
                  type="text"
                />
              </div>
              <div className={cComponent.comment}>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  name="description"
                  className={cComponent.commentInput}
                  type="text"
                />
              </div>
          ...
          {!loadingCom ? (
            comments && comments.length > 0 ? (
              comments.map((comment) => (
                <div
                  key={comment.item_comment_id}
                  className={cComponent.comentario}
                >
                  ...
                  {localStorage.getItem("N4jQctA") && (
                    <button
                      className="btn"
                      onClick={(e) =>
                        handleCommentDelete(e, comment.item_comment_id)
                      }
                    >
                      <DeleteIcon />
                    </button>
              ...
              <div className={cComponent.comentario}>
                <div className={cComponent.cuerpoComentario}>
                  <h5>
                    No hay comentarios acerca de este producto aún, sé el
                    primero.{" "}
                  </h5>
                </div>
              </div>
